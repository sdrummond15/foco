...
class CoursesControllerCourses extends JControllerForm
{
...
class PHP_email extends PHPMailer
{
    ...
    public static function email_confirma_contrato($dataForm)
    {
        ...
        $mail->IsHTML();
        $mail->setSubject("Inscrito no Curso");
        $mail->setBody(
            '<html>'
            . '<body>'
            . '<p>Inscrito no curso: ' . $course[0]->name . ' com sucesso.</p>'
        ...
        $mail->setSender(array($mailfrom, $fromname));
        $mail->IsHTML();
        $mail->setSubject("Inscrição Curso");
        $mail->setBody(
            '<html>'
            . '<body>'
            . '<p>Inscrição no curso: ' . $course[0]->name . ' com sucesso.</p>'
